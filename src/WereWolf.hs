...
ordMovements m tir wereWolf = sortBy compares (getDistance tir moves)
  where
    moves = getPossibleMoves m wereWolf

compares :: (Ord a, Ord b, Ord c) => (a, b, c) -> (a, b, c) -> Ordering
compares (a1, b1, c1) (a2, b2, c2)
  | c1 > c2 = GT -- Greater Than
  | c1 == c2 = EQ -- EQUAL
...
getPossibleMoves m (cx, cy) =
  [ (x, y) | (x, y) <- (allPositions m (cx, cy)), ( (not $ checkIsExit m (x, y))
...
euclideanDistance (tx, ty) (wx, wy) = (wx, wy, sqrt (((wxf - txf) ** 2) + ((wyf - tyf) ** 2)))
  where
    wxf = fromInteger $ toInteger wx :: Float
